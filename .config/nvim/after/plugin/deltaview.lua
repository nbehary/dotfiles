...
-- (migrated from the now-archived delta.lua; uses only the :Delta command that deltaview.nvim provides)
vim.keymap.set('n', '<leader>dv', function()
  ...
  for _, f in ipairs(changed) do
    ...
    if vim.api and vim.api.nvim_out_write then
      vim.api.nvim_out_write = function() end
    end
    if vim.api and vim.api.nvim_err_writeln then
    ...
    if vim.api and orig_out_write then
      vim.api.nvim_out_write = orig_out_write
    end
    if vim.api and orig_err_writeln then
      vim.api.nvim_err_writeln = orig_err_writeln
    end
    if vim.api and orig_echo then
